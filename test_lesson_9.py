...
db_connection_string = "postgresql://postgres:15121501@localhost:5432/QA"
...
def test_get_table():
    ...
    assert names[2] == 'users'

def test_insert():
    db = create_engine(db_connection_string)
    sql = text("insert into users(\"user_email\") values (:new_user_email)")
    with db.connect() as connection:  
        connection.execute(sql, {"new_user_email": 'alekseeva@mail.ru'})
        connection.commit()
...
def test_update():
    ...
    with db.connect() as connection:  #
        ...
        connection.commit()
        print(f"Данные пользователя изменены")
